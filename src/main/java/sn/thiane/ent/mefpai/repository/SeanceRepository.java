...
public interface SeanceRepository extends JpaRepository<Seance, Long> {
    ...
    @Query(
        value = "select distinct seance from Seance seance left join fetch seance.cours left join fetch seance.salle left join fetch seance.groupe",
        countQuery = "select count(distinct seance) from Seance seance"
    )
    Page<Seance> findAllWithToOneRelationships(Pageable pageable);

    @Query(
        "select distinct seance from Seance seance left join fetch seance.cours left join fetch seance.salle left join fetch seance.groupe"
    )
    List<Seance> findAllWithToOneRelationships();

    @Query(
        "select seance from Seance seance left join fetch seance.cours left join fetch seance.salle left join fetch seance.groupe where seance.id =:id"
    )
